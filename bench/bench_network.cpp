...
public:
    ...
    ~udp_broadcast_socket() { if (fd_ >= 0) ::close(fd_); }
...
// ============================================================================
// L2 raw socket send over network interface
// ============================================================================

static void BM_L2_Network_Send_Small(benchmark::State& state) {
    if (!can_run_network_benchmarks()) {
        state.SkipWithError("requires root and network interface");
        return;
    }

    auto iface = get_network_interface();
    if (!iface.has_value()) {
        state.SkipWithError("no network interface available");
        return;
    }
    ...
    auto frame = l2net::build_simple_frame(
        ...
        iface->mac(),
        l2net::constants::eth_p_custom,
        std::vector<std::uint8_t>(50, 0x42)
    );

    if (!frame.has_value()) {
        state.SkipWithError("failed to build frame");
        return;
    }
...
static void BM_L2_Network_Send_Large(benchmark::State& state) {
    ...
    auto frame = l2net::build_simple_frame(
        ...
        iface->mac(),
        l2net::constants::eth_p_custom,
        std::vector<std::uint8_t>(1400, 0x42)
    );
...
// ============================================================================
// L2 VLAN tagged send over network interface
// ============================================================================
...
static void BM_UDP_Network_Broadcast_Small(benchmark::State& state) {
    ...
    state.SetBytesProcessed(state.iterations() * 50);
}
...
// ============================================================================
// parameterized payload size benchmarks
// ============================================================================
...
BENCHMARK(BM_L2_Network_PayloadSize)
    ->Arg(32)
    ->Arg(64)
    ->Arg(128)
    ->Arg(256)
...
BENCHMARK(BM_UDP_Network_PayloadSize)
    ->Arg(32)
    ->Arg(64)
    ->Arg(128)
    ->Arg(256)
