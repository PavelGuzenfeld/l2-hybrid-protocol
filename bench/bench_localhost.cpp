...
namespace bench
{
    ...
    void run_localhost_benchmarks()
    {
        ...
        auto tagged_frame = l2net::build_vlan_frame(l2net::mac_address::null(), l2net::mac_address::null(), tci, 0x88B5, std::vector<std::uint8_t>(1386, 0x42));
        ...
        if (can_run_raw_benchmarks())
        {
            ...
            if (channel.has_value())
            {
                ...
                Bench().batch(small_payload.size()).run("L2_Send_Small", [&]
                                                        { (void)channel->send(small_payload); });

                std::vector<std::uint8_t> large_payload(1400, 0x42);
                Bench().batch(large_payload.size()).run("L2_Send_Large", [&]
                                                        { (void)channel->send(large_payload); });

                std::vector<std::uint8_t> jumbo_payload(8000, 0x42);
                Bench().batch(jumbo_payload.size()).run("L2_Send_Jumbo", [&]
                                                        { (void)channel->send(jumbo_payload); });
            }
        }
        else
        {
        ...
        // UDP Comparisons
        {
            ...
            {
                std::array<std::uint8_t, 50> small_payload{};
                Bench().batch(small_payload.size()).run("UDP_Send_Small", [&]
                                                        { (void)sock.send_to(small_payload.data(), small_payload.size(), 19999); });

                std::vector<std::uint8_t> large_payload(1400, 0x42);
                Bench().batch(large_payload.size()).run("UDP_Send_Large", [&]
                                                        { (void)sock.send_to(large_payload.data(), large_payload.size(), 19999); });

                std::vector<std::uint8_t> jumbo_payload(8000, 0x42);
                Bench().batch(jumbo_payload.size()).run("UDP_Send_Jumbo", [&]
