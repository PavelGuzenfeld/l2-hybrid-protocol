...
public:
    ...
    ) noexcept -> result<hybrid_endpoint>;

    // client: connect to server and exchange MACs
    [[nodiscard]] static auto create_client(
        interface_info const& iface,
    ...
    [[nodiscard]] auto send_data(std::span<std::uint8_t const> payload) noexcept
        -> void_result;

    [[nodiscard]] auto send_data(std::string_view payload) noexcept
        -> void_result;

    // blocking receive
    [[nodiscard]] auto receive_data() noexcept
        -> result<data_message>;

    // start background receiver
    [[nodiscard]] auto start_receiver(message_callback callback) noexcept
        -> void_result;
    ...
    [[nodiscard]] auto send_loop(
        std::function<std::vector<std::uint8_t>()> message_generator
    ) noexcept -> void_result;
    ...
    [[nodiscard]] auto peer() const noexcept -> mac_address const& { return peer_mac_; }
    [[nodiscard]] auto interface() const noexcept -> interface_info const& { return interface_; }
    [[nodiscard]] auto config() const noexcept -> hybrid_config const& { return config_; }
    [[nodiscard]] auto is_running() const noexcept -> bool { return running_.load(); }
...
[[nodiscard]] auto run_server(
    std::uint16_t port,
    mac_address const& local_mac,
    std::chrono::seconds timeout = std::chrono::seconds{30}
) noexcept -> result<mac_address>;

[[nodiscard]] auto run_client(
    std::string_view server_ip,
    std::uint16_t port,
    mac_address const& local_mac,
    std::chrono::seconds timeout = std::chrono::seconds{30}
