...
public:
    ...
    // factory
    [[nodiscard]] static auto create(ipc_config const& config = {}) noexcept
        -> result<ipc_channel>;
    ...
    [[nodiscard]] auto send(std::string_view message) noexcept
        -> result<std::size_t>;
    ...
    [[nodiscard]] auto try_receive() noexcept
        -> result<std::optional<std::vector<std::uint8_t>>>;

    // receive loop with callback
    [[nodiscard]] auto receive_loop(message_callback const& callback) noexcept
        -> void_result;
...
[[nodiscard]] auto create_ipc_pair(ipc_config const& config = {}) noexcept
    -> result<ipc_pair>;
