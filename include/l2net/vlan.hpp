...
namespace l2net
{
    ...
    struct vlan_tci
    {
        ...
        {
            return static_cast<std::uint16_t>((static_cast<std::uint16_t>(priority) << 13) |
                                              (static_cast<std::uint16_t>(dei ? 1 : 0) << 12) | (vlan_id & 0x0FFF));
        }

        // decode from 16-bit TCI value
        [[nodiscard]] static constexpr auto decode(std::uint16_t const tci) noexcept -> vlan_tci
        {
            return vlan_tci{.priority = static_cast<std::uint8_t>((tci >> 13) & 0x07),
                            .dei = ((tci >> 12) & 0x01) != 0,
    ...
    [[nodiscard]] auto build_vlan_frame(mac_address const &dest, mac_address const &src, vlan_tci const &tci,
                                        std::uint16_t inner_ether_type, std::span<std::uint8_t const> payload) noexcept
        -> result<std::vector<std::uint8_t>>;

    [[nodiscard]] auto build_vlan_frame(mac_address const &dest, mac_address const &src, vlan_tci const &tci,
                                        std::uint16_t inner_ether_type, std::string_view payload) noexcept
        -> result<std::vector<std::uint8_t>>;
