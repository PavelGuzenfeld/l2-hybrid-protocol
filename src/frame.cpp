...
namespace l2net
{
    ...
    auto frame_parser::parse(std::span<std::uint8_t const> data) noexcept -> bool
    {
        ...
        auto const type_field = static_cast<std::uint16_t>((static_cast<std::uint16_t>(data[12]) << 8) | data[13]);
    ...
    auto build_simple_frame(mac_address const &dest, mac_address const &src, std::uint16_t const ether_type,
